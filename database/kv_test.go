...
// 	fmt.Println("Pages:", db.kv.page.flushed)
//     fmt.Println("Root :", db.kv.tree.root)
// }

func TestMultipleSecondaryIndexes(t *testing.T) {
	testDBPath := "test_multi_index.db"
	//defer os.Remove(testDBPath)
	...
	fmt.Println("Table Prefix:", users.Prefix)
    fmt.Println("Index Prefixes:", users.IndexPrefix)

	// Insert 10 rows
	for i := int64(1); i <= 50; i++ {
		rec := Record{}
		rec.AddInt64("id", i)
		rec.AddStr("name", []byte(fmt.Sprintf("User%d", i)))
		rec.AddInt64("age", 20+i)
	...
	// Update one indexed column (name + age)
	rec := Record{}
	rec.AddInt64("id", 5)
	rec.AddStr("name", []byte("UpdatedUser"))
	rec.AddInt64("age", 99)
	...
	if string(search.Get("name").Str) != "UpdatedUser" {
		t.Fatal("name not updated")
	}

	if search.Get("age").I64 != 99 {
		t.Fatal("age not updated")
	}
